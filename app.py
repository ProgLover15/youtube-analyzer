...
CLIENT_SECRETS_FILE = {
    "web": {
        ...
        "redirect_uris": ["https://youtube-analyzer-cpan.onrender.com/auth/callback"] # RenderのURLに合わせる
    }
}
...
def auth_status():
    return jsonify({"status": "authenticated"}) if 'credentials' in session else (jsonify({"error": "Not authenticated"}), 401)
...
def get_all_channels():
    ...
    try:
        while True:
            res = youtube.subscriptions().list(part="snippet", mine=True, maxResults=50, pageToken=next_page_token).execute()
            for item in res.get("items", []):
                all_subscriptions.append({
                    ...
                    "lastUploadDate": "pending", "isSubscribed": True,
                })
...
def analyze_channel():
    youtube = build_youtube_service()
    if not youtube: return jsonify({"error": "Not authenticated"}), 401
    channel_id = request.get_json().get('channelId')
    try:
        c_res = youtube.channels().list(part="contentDetails", id=channel_id).execute()
        playlist_id = c_res["items"][0]["contentDetails"]["relatedPlaylists"]["uploads"]
        p_res = youtube.playlistItems().list(part="snippet", playlistId=playlist_id, maxResults=1).execute()
        last_date = p_res["items"][0]["snippet"]["publishedAt"] if p_res.get("items") else None
        return jsonify({"channelId": channel_id, "lastUploadDate": last_date})
    except: return jsonify({"channelId": channel_id, "lastUploadDate": None})
